...
def generate_udev_rules_str():
    ...
    etc_net_file_str = "\n".join(['KERNEL=="eth0", GOTO="no_eth0_config"', 'SUBSYTEM=="net", ACTION=="add", KERNEL=="eth*", RUN+="/sbin/ifconfig %k up", RUN+="/sbin/dhclient -v %k -lf /var/run/dhclient--%.lease -pf /var/run/dhclient--%k.pid"', '', 'LABEL="no_eth0_config"'])
