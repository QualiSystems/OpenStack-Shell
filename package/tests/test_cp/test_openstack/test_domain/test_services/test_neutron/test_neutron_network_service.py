...
class TestNeutronNetworkService(TestCase):
    ...
    def test_create_or_get_network_with_vlanid_no_conflict(self):
        """
        ...
        result = self.network_service.create_or_get_network_with_vlanid(openstack_session=self.openstack_session,
                                                               vlanid=test_vlanid,
                                                               logger=self.mock_logger)

        self.assertEqual(result, 'test_network')

    def test_create_or_get_network_with_vlaniid_conflict(self):
    ...
    def test_get_network_with_vlanid_valid_network(self):
        ...
        result = self.network_service.get_network_with_vlanid(openstack_session=self.openstack_session,
                                                              vlanid=test_vlanid,
                                                              logger=self.mock_logger)
        self.assertEqual(result, 'test_network')


    def test_get_network_with_vlanid_no_network(self):
        test_vlanid = '42'
        mock_client = Mock()
        test_neutron_network_service.neutron_client.Client = Mock(return_value=mock_client)
